package com.example.llamadrama.data.repository
...
class TeamRepository(private val database: AppDatabase) {
    ...
    suspend fun addMeetingNote(memberId: Long, content: String): Long {
        val now = LocalDateTime.now()
        val note = MeetingNote(
            memberId = memberId,
            timestampEpochSecond = now.toEpochSecond(ZoneOffset.UTC),
            content = content
        )
        ...
        member?.let {
            teamMemberDao.update(
                it.copy(
                    lastContactEpochDay = LocalDate.now().toEpochDay(),
